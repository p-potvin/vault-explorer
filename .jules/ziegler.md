...
## 2024-06-05 - Missing Input Validation in IPC Handlers
**Vulnerability:** Several IPC handlers in `main.js` (`get-everything-size`, `scan-directory`, `scan-specific-files`, `get-trickplay-sprites`, `get-file-size`, `rename-file`, `open-file`, `show-in-folder`) were missing strict input validation. They assumed inputs like file paths were valid strings and existed on the disk.
**Learning:** Failing to validate the type and existence of user-provided paths or inputs before passing them to native Node modules (like `fs` or `child_process`) can lead to unexpected crashes, unhandled exceptions, and potentially arbitrary file read/write or execution vulnerabilities, depending on how the input is consumed downstream.
**Prevention:** All IPC handlers accepting user input must strictly validate the expected type (e.g., `typeof arg === 'string'`) and, if the input is a path, verify its existence (e.g., `fs.existsSync(path)`) before processing to prevent crashes and ensure security.
